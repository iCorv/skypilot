    ...
    * for provisioning on a cloud
    ...
    def cpus(self) -> Optional[str]:
        ...
        if self.cloud is not None and self._instance_type is not None:
            vcpus, _ = self.cloud.get_vcpus_mem_from_instance_type(self._instance_type)
            return str(vcpus)
        return None
    ...
    def _try_validate_cpus_mem(self) -> None:
        ...
        if self._instance_type is not None:
            ...
            cpus, mem = self.cloud.get_vcpus_mem_from_instance_type(self._instance_type)
            if self._cpus is not None:
    ...
    def _try_validate_image_id(self) -> None:
        ...
        try:
            self._cloud.check_features_are_supported(
                self, requested_features={clouds.CloudImplementationFeatures.IMAGE_ID}
            )
        except exceptions.NotSupportedError as e:
            with ux_utils.print_exception_no_traceback():
                raise ValueError(
                    "image_id is only supported for AWS/GCP/Azure/IBM/OCI/"
                    "Kubernetes, please explicitly specify the cloud."
                ) from e
            ...
            try:
                ...
                with ux_utils.print_exception_no_traceback():
                    raise ValueError(
                        ...
                        f"for instance type {self.instance_type}."
                    ) from None
    ...
    def _try_validate_labels(self) -> None:
        ...
        if self.cloud is None:
            ...
            with ux_utils.print_exception_no_traceback():
                raise ValueError("Cloud must be specified when labels are provided.")
    ...
    def make_deploy_variables(
        ...
        if (
            ...
            and self.accelerators is not None
        ):
    ...
    def less_demanding_than(
        ...
        if self.disk_tier is not None:
            ...
            if self.disk_tier != resources_utils.DiskTier.BEST:
                ...
                if not (
                    self.disk_tier <= other.disk_tier
    ...
    def get_required_cloud_features(self) -> Set[clouds.CloudImplementationFeatures]:
        ...
        if (
            self.disk_tier is not None
            and self.disk_tier != resources_utils.DiskTier.BEST
        ):
    ...
    def from_yaml_config(
        ...
        if config and accelerators is not None:
            ...
            if (
                len(accelerators) > 1
                and any_of_configs
                and not isinstance(accelerators, set)
            ):
                with ux_utils.print_exception_no_traceback():
                    raise ValueError(
    ...
    def _from_yaml_config_single(cls, config: Dict[str, str]) -> "Resources":
        ...
        if config.get("spot_recovery") is not None:
            logger.warning(
                "spot_recovery is deprecated. Use job_recovery "
    ...
    def to_yaml_config(self) -> Dict[str, Union[str, int]]:
        ...
        add_if_not_none("_cluster_config_overrides", self._cluster_config_overrides)
