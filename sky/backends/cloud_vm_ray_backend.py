...
def write_ray_up_script_with_patched_launch_hash_fn(
    ...
    """
    with open(
        _RAY_UP_WITH_MONKEY_PATCHED_HASH_LAUNCH_CONF_PATH, "r", encoding="utf-8"
    ) as f:
...
class FailoverCloudErrorHandlerV1:
    ...
    @staticmethod
    def _handle_errors(
        stdout: str, stderr: str, is_error_str_known: Callable[[str], bool]
    ) -> List[str]:
        ...
        if "rsync: command not found" in stderr:
            with ux_utils.print_exception_no_traceback():
                ...
                setattr(e, "detailed_reason", f"stdout: {stdout}\nstderr: {stderr}")
                raise e
        detailed_reason = textwrap.dedent(
            f"""\
        ====== stdout ======
        {stdout}
        ====== stderr ======
        ...
        print(stderr)
        with ux_utils.print_exception_no_traceback():
            e = RuntimeError("Errors occurred during provision; " "check logs above.")
    ...
    def _scp_handler(
        ...
        del zones  # Unused.
        errors = FailoverCloudErrorHandlerV1._handle_errors(
            stdout, stderr, is_error_str_known=lambda x: "SCPError:" in x.strip()
        )
    ...
    def _ibm_handler(
        ...
        errors = FailoverCloudErrorHandlerV1._handle_errors(
            stdout, stderr, lambda x: "ERR" in x.strip() or "PANIC" in x.strip()
        )
    ...
    def _oci_handler(
        ...
        errors = FailoverCloudErrorHandlerV1._handle_errors(
            stdout,
            stderr,
            lambda x: "VcnSubnetNotFound" in x.strip()
            or (
                "oci.exceptions.ServiceError" in x.strip()
                and any(known_err in x.strip() for known_err in known_service_errors)
            ),
        )
        logger.warning(f"Got error(s) in {region.name}:")
        messages = "\n\t".join(errors)
    ...
    def _gcp_handler(
        ...
        for e in errors:
            ...
            if code in ("QUOTA_EXCEEDED", "quotaExceeded"):
                ...
                if (
                    launchable_resources.accelerators is not None
                    and any(
...
class RetryingVmProvisioner(object):
    ...
    def _retry_zones(
        ...
        for zones in self._yield_zones(
            ...
            try:
                ...
                # invalid.
                _add_to_blocked_resources(
                    self._blocked_resources, to_provision.copy(region=None, zone=None)
                )
                raise exceptions.ResourcesUnavailableError(
    ...
    def provision_with_retries(
        ...
        while True:
            if (
                isinstance(to_provision.cloud, clouds.Azure)
                and to_provision.accelerators is not None
                and "A10" in to_provision.accelerators
            ):
                logger.warning(
                    f"{style.BRIGHT}{fore.YELLOW}Trying to launch "
            ...
            try:
                ...
                if (
                    isinstance(to_provision.cloud, (clouds.Kubernetes, clouds.RunPod))
                    and controller_utils.Controllers.from_name(cluster_name) is not None
                ):
                    assert (
                        clouds.CloudImplementationFeatures.STOP in requested_features
                    ), requested_features
                    requested_features.remove(clouds.CloudImplementationFeatures.STOP)
...
class CloudVmRayResourceHandle(backends.backend.ResourceHandle):
    ...
    def get_command_runners(
        self, force_cached: bool = False, avoid_ssh_control: bool = False
        ...
        updated_to_skypilot_provisioner_after_provisioned = (
            self.launched_resources.cloud.PROVISIONER_VERSION
            >= clouds.ProvisionerVersion.SKYPILOT
            and self.cached_external_ips is not None
            and self.cached_cluster_info is None
        )
        ...
        if (
            ...
            >= self.launched_resources.cloud.PROVISIONER_VERSION
            or updated_to_skypilot_provisioner_after_provisioned
        ):
...
class CloudVmRayBackend(backends.Backend["CloudVmRayResourceHandle"]):
    ...
    def check_resources_fit_cluster(
        ...
        if valid_resource is None:
            for example_resource in task.resources:
                ...
                if (
                    example_resource.requires_fuse
                    and not launched_resources.requires_fuse
                ):
    ...
    def _provision(
        ...
        with timeline.FileLockEvent(lock_path):
            ...
            if "provision_record" in config_dict:
                ...
                handle.update_cluster_ips(
                    max_attempts=_FETCH_IP_MAX_ATTEMPTS, cluster_info=cluster_info
                )
                handle.update_ssh_ports(max_attempts=_FETCH_IP_MAX_ATTEMPTS)
            ...
            with rich_utils.safe_status("[bold cyan]Preparing SkyPilot runtime"):
                ...
                self.run_on_head(
                    handle, instance_setup.MAYBE_SKYLET_RESTART_CMD, source_bashrc=True
                )
    ...
    def _update_after_cluster_provisioned(
        ...
        if open_new_ports:
            cloud = handle.launched_resources.cloud
            if not (cloud.OPEN_PORTS_VERSION <= clouds.OpenPortsVersion.LAUNCH_ONLY):
    ...
    def _setup(
        ...
        def _setup_node(node_id: int) -> None:
            ...
            if detach_setup or _is_command_length_over_limit(encoded_script):
                ...
                create_script_code = (
                    f"{{ echo {encoded_script} > " f"{remote_setup_file_name}; }}"
                )

            if detach_setup:
                return
            setup_log_path = os.path.join(self.log_dir, f"setup-{runner.node_id}.log")
    ...
    def _exec_code_on_head(
        ...
        mkdir_code = f"{cd} && mkdir -p {remote_log_dir} && " f"touch {remote_log_path}"
        ...
        if _is_command_length_over_limit(job_submit_cmd):
            ...
            with tempfile.NamedTemporaryFile("w", prefix="sky_app_") as fp:
                ...
                head_runner.rsync(
                    source=fp.name, target=script_path, up=True, stream_logs=False
                )
        ...
        if managed_job_dag is not None:
            ...
            managed_job_code = managed_job_codegen.set_pending(job_id, managed_job_dag)
        ...
        try:
            if not detach_run:
                if (
                    handle.cluster_name
                    in controller_utils.Controllers.JOBS_CONTROLLER.value.candidate_cluster_names
                ):
    ...
    def tail_logs(
        ...
        code = job_lib.JobLibCodeGen.tail_logs(
            job_id, managed_job_id=managed_job_id, follow=follow
        )
    ...
    def teardown_no_lock(
        ...
        if (
            ...
            try:
                ...
                self.run_on_head(handle, f"{constants.SKY_RAY_CMD} stop --force")
            except exceptions.FetchClusterInfoError:
            ...
            try:
                provisioner.teardown_cluster(
                    repr(cloud),
                    resources_utils.ClusterName(cluster_name, cluster_name_on_cloud),
                    terminate=terminate,
                    provider_config=config["provider"],
                )
            except Exception as e:  # pylint: disable=broad-except
    ...
    def set_autostop(
        ...
        if idle_minutes_to_autostop is not None:
            ...
            if (
                isinstance(
                    handle.launched_resources.cloud, (clouds.Kubernetes, clouds.RunPod)
                )
                and not down
                and idle_minutes_to_autostop >= 0
            ):
                ...
                logger.info(
                    "Auto-stop is not supported for Kubernetes "
    ...
    def is_definitely_autostopping(
        ...
        logger.debug(
            "Failed to check if cluster is autostopping with "
    ...
    def _check_existing_cluster(
        ...
        if prev_cluster_status is not None:
            ...
            all_ports = resources_utils.port_set_to_ranges(
                current_ports_set | requested_ports_set
            )
            to_provision = handle.launched_resources
            if (
                to_provision.cloud.OPEN_PORTS_VERSION
                <= clouds.OpenPortsVersion.LAUNCH_ONLY
            ):
    ...
    def _get_task_env_vars(
        self, task: task_lib.Task, job_id: int, handle: CloudVmRayResourceHandle
    ) -> Dict[str, str]:
