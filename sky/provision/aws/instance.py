...
def _default_ec2_resource(region: str) -> Any:
    ...
    return aws.resource("ec2", region_name=region, max_attempts=BOTO_MAX_RETRIES)
...
def _ec2_call_with_retry_on_server_error(
    ec2_fail_fast_fn: Callable[..., _T], log_level=logging.DEBUG, **kwargs
) -> _T:
...
def _create_instances(
    ...
    conf.update({"MinCount": count, "MaxCount": count, "TagSpecifications": tag_specs})
    ...
    max_tries = max(
        num_subnets * (BOTO_CREATE_MAX_RETRIES // num_subnets), len(subnet_ids)
    )
...
def run_instances(
    ...
    def _create_node_tag(target_instance, is_head: bool = True) -> str:
        node_type_tags = (
            constants.HEAD_NODE_TAGS if is_head else constants.WORKER_NODE_TAGS
        )
        node_tag = [{"Key": k, "Value": v} for k, v in node_type_tags.items()]
        if is_head:
            node_tag.append(
                {"Key": "Name", "Value": f"sky-{cluster_name_on_cloud}-head"}
            )
        else:
            node_tag.append(
    ...
    to_start_count = config.count - len(running_instances) - len(pending_instances)
    ...
    if (
        config.resume_stopped_nodes
        and to_start_count > 0
        and (stopping_instances or stopped_instances)
    ):
        ...
        while (
            stopping_instances
            and to_start_count > len(stopped_instances)
            and time.time() - time_start < _RESUME_INSTANCE_TIMEOUT
        ):
            ...
            with pool.ThreadPool(processes=1) as pool_:
                ...
                while (
                    time.time() - per_instance_time_start < _RESUME_PER_INSTANCE_TIMEOUT
                ):
                    if fut.ready():
                        fut.get()
                        break
        ...
        if tags:
            ...
            ec2.meta.client.create_tags(
                Resources=resumed_instance_ids, Tags=_format_tags(tags)
            )
    ...
    if to_start_count > 0:
        ...
        if zone is None:
            ...
            logger.warning(
                "Newly created instances are in zone "
                f"{placement_zone}, "
                f"while previous instances are in zone {zone}."
            )
...
def _filter_instances(
    ...
    if included_instances is not None and excluded_instances is not None:
        raise ValueError(
            '"included_instances" and "exclude_instances"'
...
def query_instances(
    ...
    instances = _filter_instances(
        ec2, filters, included_instances=None, excluded_instances=None
    )
    status_map = {
...
def open_ports(
    ...
    if not instance_list:
        with ux_utils.print_exception_no_traceback():
            raise ValueError(
                "Instance with cluster name " f"{cluster_name_on_cloud} not found."
            )
    sg = _get_sg_from_name(ec2, sg_name)
    if sg is None:
        with ux_utils.print_exception_no_traceback():
            raise ValueError(
                "Cannot find new security group "
...
def cleanup_ports(
    ...
    sg = _get_sg_from_name(ec2, sg_name)
    if sg is None:
        logger.warning("Find security group failed. Skip cleanup security group.")
        return
